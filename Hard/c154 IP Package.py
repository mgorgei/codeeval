...
А novice hacker decided to play a trick on your friends and changed addresses in
the header of an IP packet, you need to fix all the packets and put the correct
address to help your friends.

A package consists of a header IPV4 and a body of the package.

You need to replace the IP source and destination IP address to the new
addresses and print out the IP header under the condition that the package is
valid. This means you should calculate checksum of IP header.

INPUT SAMPLE:
Your program should accept as its first argument a path to a filename.
Each line contains new source ip address, new destination ip address and package
in hex.
...
OUTPUT SAMPLE:
For each line of input print out ip header with new destination and source ip
address and valid checksum
